...
if(!isset($_GET['genre'])){
    $sql = "SELECT * FROM movies as m JOIN (SELECT id FROM movies WHERE isSeries = " . $type ." ORDER BY RAND() LIMIT 1 ) as i ON m.id = i.id";
}
else{
    $sql = "SELECT * FROM movies as m JOIN (SELECT id FROM movies WHERE isSeries = " . $type ." AND movieGenreId = " . $_GET['genre'] . " ORDER BY RAND() LIMIT 1 ) as i ON m.id = i.id";
}
...
<div class="spotlight-container">
    ...
    <?php
        ...
        if(!isset($_GET['genre'])){
            $sql = "SELECT * FROM watch_lists as wl JOIN movies as m ON wl.movieId = m.id WHERE profileId = " . $profile['id'] . " AND isSeries = " . $type ." ORDER BY wl.id DESC LIMIT 6";
        }
        else{
            $sql = "SELECT * FROM watch_lists as wl JOIN movies as m ON wl.movieId = m.id WHERE profileId = " . $profile['id'] . " AND movieGenreId = " . $_GET['genre']. " AND isSeries = " . $type ." ORDER BY wl.id DESC LIMIT 6";
        }
